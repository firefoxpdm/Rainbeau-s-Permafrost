	...
	public class Watcher : MapComponent {
		...
		public override void MapComponentTick() {
			...
			for (int i = 0; i < num; i++) {
				...
				if (cellTemp < 0f) {
					if (terrainDef.defName == "WaterShallow" || terrainDef.defName == "WaterMovingShallow" || terrainDef.defName == "IceST" || terrainDef.defName == "IceSMT") {
						int freezable = 0;
						for (int k = -1; k < 2; k++) {
							for (int j = -1; j < 2; j++) {
								int x = c.x + k;
						...
						if (Rand.Value < shallowChance * freezable) {
							...
							else if (terrainDef.defName == "WaterMovingShallow") { iceType = "IceSMT"; }
							else if (terrainDef.defName == "IceST") { iceType = "IceS"; }
							else { iceType = "IceSM"; }
							map.terrainGrid.SetTerrain (c, TerrainDef.Named(iceType));
						}
					}
					if (terrainDef.defName == "WaterDeep" || terrainDef.defName == "WaterMovingChestDeep" || terrainDef.defName == "IceDT" || terrainDef.defName == "IceDMT") {
						int freezable = 0;
						for (int k = -1; k < 2; k++) {
							for (int j = -1; j < 2; j++) {
								int x = c.x + k;
						...
						if (Rand.Value < deepChance * freezable) {
							...
							else if (terrainDef.defName == "WaterMovingChestDeep") { iceType = "IceDMT"; }
							else if (terrainDef.defName == "IceDT") { iceType = "IceD"; }
							else { iceType = "IceDM"; }
							map.terrainGrid.SetTerrain (c, TerrainDef.Named(iceType));
						}
					}
				...
				if (cellTemp > 0f) {
					if (terrainDef.defName.Contains("IceS") || terrainDef.defName.Contains("IceD")) {
						int quickThaw = 1;
						for (int k = -1; k < 2; k++) {
							for (int j = -1; j < 2; j++) {
								int x = c.x + k;
						...
						if (Rand.Value < (shallowChance * quickThaw)) {
							...
							else if (terrainDef.defName == "IceD") { waterType = "IceDT"; }
							else if (terrainDef.defName == "IceSMT") { waterType = "WaterMovingShallow"; }
